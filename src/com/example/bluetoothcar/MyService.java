...
public class MyService {
    ...
    // 构造器
    public MyService(Context context, Handler handler,Handler atHandler) {
        btAdapter = BluetoothAdapter.getDefaultAdapter();
        myState = STATE_NONE;
        myHandler = handler;
        autoHandler = atHandler;
    }
    //设置当前连接状态的方法
    private synchronized void setState(int state) {
        myState = state;
        myHandler.obtainMessage(MainActivity.State_Change, state, -1).sendToTarget();
    }
    //获取当前连接状态的方法
    ...
    public synchronized void connected(BluetoothSocket socket, BluetoothDevice device) {
        ...
        Message msg = myHandler.obtainMessage(MainActivity.Device_Name);
        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    myHandler.obtainMessage(MainActivity.Message, bytes, -1, buffer)
                    .sendToTarget();
                } catch (IOException e) {
                    e.printStackTrace();
                    break;
